...
|---|------|------|------|
| 17 | 甩尾巴 | 每 0.6s | 尾巴 `|_)` ↔ `|~)` |
| 18 | 耳朵抖 | 随机（20%/2.5s） | 耳朵 `/\` → `/╲` |
| 19 | 瞳孔放大 | 随机（15%/4s，idle） | 眼睛 `o.o` → `@.@` |
| 20 | 踩奶 | idle 状态 | 爪子交替 `/||  |\` ↔ `/|  ||\` |
| 21 | 呼噜气泡 | busy/thinking | 12条呼噜语轮换（purrr~/mrrrow~/nyaa~） |
| 22 | 拖拽炸毛 | 拖拽中 | 耳朵炸毛 `/╲╱╲` + 惊恐脸 `>.<` |
...
| 2 | **天降炸弹** | idle 时 10% 概率替代走路 | 引线 ✦/◌ 燃烧 + 倒计时 ³·→²·→¹· → 红/橙/黄闪烁爆炸 + `ᵇᵒᵒᵐ~💥` → 重组 |
...
| 道具 | 尺寸 | 触发 | 位置 | 描述 |
|------|------|------|------|------|
| **显示器** | 16×5 | busy（65%） | side-right | 终端显示 `~$ᵒᵖᵉⁿᶜᵒᵈᵉ` + 20帧随机火星文状态（thinking/writing/git/bug/npm/compile/test/refactor/deploy/merge/lint/format/review/oops/hmm/help） |
| **Pad** | 18×9 | busy（35%）+ idle（40%） | front（角色隐藏） | 迷你小人在屏幕里玩游戏：贪吃蛇/俄罗斯方块/2048，又菜又爱玩玩两下就 game over，15帧循环 |
| **立体箱子** | 14×8 | idle（5%，每分钟最多1次）+ 首页启动 | side-left | 等轴测 3D 箱子，正面 ᵇᵒˣ 火星文。4帧循环：关闭→抖动→打开（月儿蹲着露头）→冒头 |

**busy 焦急踱步：** 显示器出现时，小人小步来回走（±3，走3秒停2秒），显示器不动。

**降落动画：** busy/idle 触发道具时，小人自动降落到工作区域（easeInQuad 500ms）。

**首页启动仪式：** 角色隐藏 → 箱子从高处加速落下（500ms）→ 2秒后摇晃 → 6秒后箱子消失 + 角色降落现身。

---

### 🔄 Phase Machine 阶段机（工作页）

`session busy` 触发的4阶段动画循环：

| 阶段 | 时长 | 动画 |
|------|------|------|
| **Phase 1** | ~15s | 小人跳上机箱 → 踱步 → 金色梯子从顶部垂下 → 爬上去 → 显示器从天花板掉落 → 小人钻入机箱（下沉3格，zIndex 降到机箱之下被遮挡） |
| **Phase 2** | 60s | 显示器显示火星文工作状态 + 上方 **vibe coding 彩色闪动**（`ᵛⁱᵇᵉ ᶜᵒᵈⁱⁿᵍ` 6色循环，300ms 切色）→ 小人从机箱左侧冒出罚站 |
| **Phase 3** | 30s | **Pad 躲猫猫**：pad 从左侧4步探出缩回序列，小人眼睛瞟左（`<_<`）+ 持续向左靠近模拟拽 pad。三 prop 同屏（机箱 + 显示器 + pad） |
| **循环** | → Phase 1 | Pad 滑出 → 小人下落（easeInQuad 500ms）→ 800ms 落地停留 → 弹跳跳回机箱 |

**电源线：** Phase 1 & 2 期间，灰色 `━` 横线从机箱右侧延伸到 sidebar 边缘（被边缘裁掉）。

**竞态保护：** `phaseSessionId` token 在新 busy/idle 事件时废弃旧回调。
...
`Ctrl+P`（或你的 leader 键）打开命令面板，输入筛选：

| 命令 | 描述 |
|------|------|
| `Mascot: Toggle visibility` | 显示/隐藏吉祥物（停止所有动画+道具） |
| `Mascot: Toggle easter eggs` | 开启/关闭彩蛋动画（小人保留，动画停止） |

**三层状态模型**：
- toggle 开 + easter 开 → 完整体验（小人+随机彩蛋）
- toggle 开 + easter 关 → 只看小人，无彩蛋
- toggle 关 → 吉祥物完全禁用

双击小人切换形象（月儿 → 包子 → 小猫 → 月儿）。
...
| 道具（显示器/Pad/箱子） | 3 |
| 道具动画帧 | 39（20+15+4） |
| Phase machine 阶段动画 | 4（跳箱+爬梯+钻入 / vibe+罚站 / pad躲猫猫 / 循环） |
| 交互操作 | 5 |
| 躲猫猫行为 | 3 |
| 随机意外 | 3 |
| busy 行为（踱步/跺脚/闪光） | 3 |
| idle 事件（Pad/箱子/睡觉） | 3 |
| 降落动画 | 2（busy/idle + 首页启动） |
| 火星文气泡 | 24（12/形象） |
| 闪色颜色 | 8 |
| 拖拽火星文 | 6 |
| Pad mini-games | 9（3游戏/形象 × 3形象） |
| **总能力** | **70+** |
